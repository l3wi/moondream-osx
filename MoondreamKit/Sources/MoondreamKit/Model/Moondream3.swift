...
// MARK: - Moondream3 Model

/// Moondream3 Vision-Language Model (Standard variant)
/// Uses standard precision vision encoder and mixed MoE language model
public class Moondream3: Module, MoondreamModel {
    @ModuleInfo(key: "vision") private var visionEncoder: VisionEncoder
    @ModuleInfo(key: "text") private var textModel: TextModel
    @ModuleInfo(key: "region") private var regionModel: RegionModel
    ...
    public init(_ config: Moondream3Configuration) {
        self.config = config
        self._visionEncoder.wrappedValue = VisionEncoder(config.vision)
        self._textModel.wrappedValue = TextModel(config.text)
        self._regionModel.wrappedValue = RegionModel(config.region)
    ...
    public func caption(
        ...
        md3Log("caption() started - length: \(length)")
        eval(pixels)
