...
/// Service for managing webcam capture on macOS
final class MacCameraService: NSObject, ObservableObject, @unchecked Sendable {
    @MainActor @Published var previewLayer: AVCaptureVideoPreviewLayer?
    @MainActor @Published var isReady = false
    @MainActor @Published var error: MacCameraError?
    ...
    private var outputDelegate: MacCameraOutputDelegate?
    ...
    private func setupSession() {
        ...
        let delegate = MacCameraOutputDelegate { [weak self] image in
            Task { @MainActor in
        ...
        sessionQueue.async { [weak self] in
            ...
            // Add camera input - macOS doesn't use position (no front/back)
            ...
            // No rotation needed on macOS - cameras are already oriented correctly
...
private final class MacCameraOutputDelegate: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate, @unchecked Sendable {
...
/// Camera errors for macOS
enum MacCameraError: LocalizedError, Identifiable, Equatable {
    ...
    var errorDescription: String? {
        switch self {
        case .permissionDenied:
            "Camera permission denied. Please enable camera access in System Settings > Privacy & Security > Camera."
        case .cameraUnavailable:
